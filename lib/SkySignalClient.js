...
export default class SkySignalClient {
	...
	/**
	 * Create an abort signal with timeout (Node 14+ compatible)
	 * AbortSignal.timeout() requires Node 17.3+, so we use AbortController for compatibility
	 * @private
	 */
	...
	async _sendRequest(endpoint, body, batchType, batch, bodySize, retryCount) {
		const timeout = this._createTimeoutSignal(this.requestTimeout);
