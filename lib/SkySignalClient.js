...
// Hoisted lookup tables — avoids creating new object literals on every _sendBatch call
 ...
 * - Intelligent batching by count and size (1MB default)
 ...
 * 1. Batch reaches `batchSize` items (default: 100)
 * 2. Batch reaches `batchSizeBytes` (default: 1MB)
 * 3. `flushInterval` timer fires (default: 60 seconds)
 *
...
export default class SkySignalClient {
	 ...
	 * @param {number} options.batchSize - Maximum items per batch before sending
	 * @param {number} options.batchSizeBytes - Maximum batch size in bytes
	 ...
	 * @param {number} options.maxBatchRetries - Maximum retry attempts for failed requests
	 * @param {number} options.traceSampleRate - Sampling rate for traces (0.0 to 1.0)
	 * @param {number} options.rumSampleRate - Sampling rate for RUM data (0.0 to 1.0)
	 */
	constructor(options = {}) {
		...
		this.batchSizeBytes = options.batchSizeBytes;
		this.flushInterval = options.flushInterval;
		this.requestTimeout = options.requestTimeout;
		this.maxRetries = options.maxBatchRetries;

		// Sampling rates
		this.traceSampleRate = options.traceSampleRate;
		this.rumSampleRate = options.rumSampleRate;
		...
		// Server-hinted config — updated from response bodies after each successful flush.
		// Collectors poll this.
		this.serverConfig = { liveQueries: null, subscriptions: null };
	 ...
	 * @param {string} metric.strategy - SERVER_MERGE | NO_MERGE | NO_MERGE_NO_HISTORY | unknown
	...
	/**
	 * Send aggregated LiveQuery rollup rows. Unlike sendLiveQueries, this sends
	 * ONE HTTP POST per flush window (not a per-item batched stream), because
	 * aggregates carry a window header.
	 *
	 * @param {Object} payload { windowStart, windowEnd, flushIntervalMs, sampleRate, aggregates }
	 * @returns {Promise<boolean>} true on 2xx success
	 */
	async sendLiveQueryAggregates(payload) {
		return this._postAggregateBatch("/api/v1/live-queries/aggregates", payload);
	}

	/**
	 * Send aggregated Subscription rollup rows. One POST per flush window.
	 *
	 * @param {Object} payload { windowStart, windowEnd, flushIntervalMs, sampleRate, aggregates }
	 * @returns {Promise<boolean>} true on 2xx success
	 */
	async sendSubscriptionAggregates(payload) {
		return this._postAggregateBatch("/api/v1/subscriptions/aggregates", payload);
	}

	/**
	 * POST an aggregate payload with the same auth/compression path as per-item
	 * batches, and parse the response for server-hinted config updates.
	 * @private
	 */
	async _postAggregateBatch(path, payload) {
		if (this.stopped) return false;
		const body = this._safeStringify(payload);
		if (!body) {
			if (this.debug) console.error(`❌ SkySignal: Failed to serialize aggregate payload for ${path}`);
			return false;
		}
		try {
			const { status, bodyText, bytesSent } = await this._postWithAuth(path, body, body.length);
			if (status >= 200 && status < 300) {
				this.stats.bytesSent += bytesSent;
				this._parseServerConfig(bodyText);
				return true;
			}
			if (this.debug) console.warn(`⚠️ SkySignal: Aggregate POST to ${path} returned ${status}: ${bodyText}`);
			return false;
		} catch (err) {
			if (this.debug) console.error(`❌ SkySignal: Aggregate POST to ${path} failed:`, err.message);
			return false;
		}
	}
	...
	stop() {
		...
		// Final flush BEFORE setting stopped flag, otherwise _sendBatch
		// bails out immediately and all pending data is silently dropped.
		this.flush();
	...
	_sendBatch(batchType, endpoint, retryCount = 0) {
		const batch = this.batches[batchType];
		this.batches[batchType] = []; // Swap reference — avoids spread copy of entire array
		...
		const body = this._safeStringify(payload);
		if (!body) {
			if (this.debug) console.error(`❌ SkySignal: Failed to serialize ${batchType} batch, skipping`);
			return;
		}
		const bodySize = body.length;

		// Don't send if client is stopped (checked before setImmediate so
		// that final-flush sends during stop() still go through)
		...
		setImmediate(() => {
			this._sendRequest(endpoint, body, batchType, batch, bodySize, retryCount)
				.catch(error => {
					...
					if (this.debug) console.error(`❌ SkySignal: Unhandled error sending ${batchType}:`, error.message);
				});
		});
	}

	/**
	 * POST body to SkySignal with auth header + gzip compression.
	 * Shared by per-item batch sends and aggregate sends.
	 * @private
	 */
	async _postWithAuth(endpoint, body, bodySize) {
		...
		return { status: response.status, bodyText, bytesSent: actualSize, url };
	}

	/**
	 * Parse a successful response body for server-hinted config.
	 * @private
	 */
	_parseServerConfig(bodyText) {
		if (!bodyText) return;
		try {
			const parsed = JSON.parse(bodyText);
			if (parsed && parsed.config) {
				if (parsed.config.liveQueries) this.serverConfig.liveQueries = parsed.config.liveQueries;
				if (parsed.config.subscriptions) this.serverConfig.subscriptions = parsed.config.subscriptions;
			}
		} catch (_e) {
			// Non-JSON or empty response is fine
		}
	}
	...
	async _sendRequest(endpoint, body, batchType, batch, bodySize, retryCount) {
		const url = `${this.endpoint}${endpoint}`;
		try {
			const { status, bodyText, bytesSent } = await this._postWithAuth(endpoint, body, bodySize);
			...
			if (status === 202) {
				...
				if (this.debug) console.warn(`⚠️ SkySignal: Unexpected response for ${batchType}:`, status, bodyText);
			...
			if (isTimeout) {
				if (this.debug) console.error(`❌ SkySignal: Request timeout for ${batchType} (${this.requestTimeout}ms) to ${url}`);
			} else if (isAbort) {
				...
				if (this.debug) console.error(`❌ SkySignal: Failed to send ${batchType} to ${url}:`, error.message);
			}
			this._handleFailedBatch(batchType, batch, retryCount);
		}
	}
	...
	_handleFailedBatch(batchType, batch, retryCount) {
		...
		if (retryCount < this.maxRetries) {
			...
			if (this.retryQueues[batchType].length >= MAX_RETRY_QUEUE_SIZE) {
				...
				if (this.debug) console.warn(`⚠️ SkySignal: Retry queue full for ${batchType}, dropping oldest batch (${dropped.batch.length} items)`);
			}
			...
			if (this.debug) console.error(`❌ SkySignal: Dropping ${batch.length} ${batchType} after ${retryCount} retries`);
		}
	}
	...
	_retryBatch(batchType) {
		...
		const body = this._safeStringify(payload);
		if (!body) {
			if (this.debug) console.error(`❌ SkySignal: Failed to serialize retry ${batchType} batch, skipping`);
			return;
		}
		const bodySize = body.length;
		...
		setImmediate(() => {
			if (!this.stopped) {
				this._sendRequest(endpoint, body, batchType, batch, bodySize, retryCount)
					.catch(error => {
						// Prevent unhandled promise rejection from crashing host app
						if (this.debug) console.error(`❌ SkySignal: Unhandled error in retry ${batchType}:`, error.message);
					});
			}
		});
	}
	 ...
	 */
	_safeStringify(obj) {
		// Fast path: try plain JSON.stringify first (no WeakSet, no replacer overhead).
		// Most APM payloads have no circular references, so this succeeds ~99% of the time.
		try {
			return JSON.stringify(obj);
		} catch (_) {
			// Fall through to safe version only if fast path fails (likely circular reference)
		}

		// Slow path: circular-reference-safe serialization
		...
		try {
			...
			if (this.debug) console.error('❌ SkySignal: JSON serialization failed:', error.message);
			return null;
		}
	}
	...
	_getEndpointForBatchType(batchType) {
		return BATCH_ENDPOINTS[batchType] || "/api/v1/traces";
	}
	...
	_getPayloadKey(batchType) {
		return BATCH_PAYLOAD_KEYS[batchType] || "data";
	}
}
