...
export default class MethodTracer {
  ...
  constructor(options = {}) {
    ...
    // Track current method context (for operation tracking)
    this._currentMethodContext = null;
  ...
  _wrapMethod(methodName, originalMethod) {
    ...
    return function(...args) {
      ...
      // Generate unique trace ID for this method execution
      // Format: methodName-timestamp-random
      const traceId = `${methodName}-${startTime}-${Math.random().toString(36).substr(2, 9)}`;
      ...
      const context = {
        ...
        // Initialize query pattern tracking for N+1 detection
        queryFingerprints: new Map(),  // fingerprint -> {count, operations, totalDuration}
        maxQueryFingerprints: 500,      // Limit unique fingerprints per method
        queryOperations: [],            // All db operations for later analysis
        maxQueryOperations: 1000,       // Limit operations per method

        // Initialize slow query tracking
        slowQueries: []                 // Slow queries for analysis
      };
            ...
            if (error) {
              ...
              try {
                ...
                if (resultStr.length > 500) {
                  methodOperation.result = JSON.parse(resultStr.substring(0, 500) + '...<truncated>"}');
                } else {
      ...
      // Set current context for operation tracking
      const previousContext = self._currentMethodContext;
      self._currentMethodContext = context;
        ...
        if (result && typeof result.then === 'function') {
          return result
            .then((resolvedValue) => {
              ...
              self._currentMethodContext = previousContext;
   ...
   */
  async _recordTrace(context, duration, error) {
    // Wait for all pending explain() queries to complete before sending trace (with timeout)
    if (context.pendingExplains && context.pendingExplains.length > 0) {
      this._log(`Waiting for ${context.pendingExplains.length} explain() queries...`);
      try {
        // Timeout pending explains after 5 seconds to avoid blocking trace recording
        const EXPLAIN_TIMEOUT_MS = 5000;
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Explain timeout')), EXPLAIN_TIMEOUT_MS)
        );

        await Promise.race([
          Promise.all(context.pendingExplains),
          timeoutPromise
        ]);
        this._log(`All explain() queries completed`);
      } catch (explainError) {
        // Silently fail - don't impact trace recording (timeout or actual error)
        this._warn(`Failed to wait for explain() queries:`, explainError.message);
      }
    }
  ...
  addOperation(operation) {
    if (this._currentMethodContext && this._currentMethodContext.operations) {
      ...
      if (operation.type === 'db' && operation.collection && operation.operation) {
        ...
        // Store full operation for later analysis (with limit check)
        ...
        if (operation.slowQuery && operation.queryAnalysis) {
          ...
          this._currentMethodContext.slowQueries.push({
            collection: operation.collection,
            operation: operation.operation,
            selector: operation.selector,
            duration: operation.duration,
  ...
  _instrumentHttpOperations() {
    ...
    try {
      ...
      this._warn("Failed to instrument HTTP operations:", error);
    }
  }
  ...
  _instrumentEmailOperations() {
    ...
    try {
      ...
      this._warn("Failed to instrument Email operations:", error);
    }
  }
  ...
  _extractAggregationIndexUsageMetrics(explainResult) {
    ...
    try {
      // For aggregations, we look at the stages array to find index usage
      const stages = explainResult.stages || [];

      // Find the first stage that used an index (usually $cursor or initial stage)
      ...
      // Look for $cursor stage (contains the initial query execution)
      const cursorStage = stages.find(stage => stage.$cursor);

      if (cursorStage && cursorStage.$cursor) {
        const queryPlanner = cursorStage.$cursor.queryPlanner;
        const executionStats = cursorStage.$cursor.executionStats;

        // Extract index from query planner
        if (queryPlanner && queryPlanner.winningPlan) {
          indexUsed = this._extractIndexFromPlan(queryPlanner.winningPlan);
        }
        ...
        // Newer MongoDB versions may have executionStats at top level
        if (explainResult.executionStats) {
          ...
          executionTimeMillis = explainResult.executionStats.executionTimeMillis || 0;
        }

        if (explainResult.queryPlanner && explainResult.queryPlanner.winningPlan) {
          indexUsed = this._extractIndexFromPlan(explainResult.queryPlanner.winningPlan);
        }
      }
      ...
      // Calculate efficiency metrics (same as find queries)
      ...
      } else if (indexUsed && indexUsed !== 'COLLSCAN') {
