...
/**
 * Module-level AsyncLocalStorage for method trace context.
 * Exported so other collectors (BullMQMonitor, SteveJobsMonitor) can read
 * the current method context to capture originatingTraceId for job-to-trace correlation.
 */
export const methodTraceContextStore = new AsyncLocalStorage();
...
export default class MethodTracer {
  ...
  constructor(options = {}) {
    ...
    // Uses the module-level store so other collectors can read the context.
    this._asyncContextStorage = methodTraceContextStore;
