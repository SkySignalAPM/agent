...
import { computeParamsSignature, buildSample } from "./aggregation/QuerySignature.js";
import { evaluateSubscription } from "./aggregation/SubscriptionPromotion.js";
import { percentile } from "./aggregation/AggregateBucket.js";
...
export default class DDPCollector {
	constructor(options = {}) {
		...
		// Aggregation mode (opt-in)
		this.ingestAggregation = options.ingestAggregation === true;
		this.subscriptionWindowStart = Date.now();
			...
			if (msgType === "sub") {
				...
				if (subId && publicationName) {
					...
					this.subscriptions.set(subId, {
						...
						_host: this.host,
						_transitionedThisWindow: "pending",
						_readyThisWindow: false,
						_wasStoppedThisWindow: false
					});
				...
				subs.forEach(subId => {
					...
					if (sub && sub.status === "pending") {
						...
						sub._transitionedThisWindow = "ready";
						sub._readyThisWindow = true;
				...
				if (sub) {
					...
					sub._transitionedThisWindow = "error";
					sub._wasStoppedThisWindow = true;
				...
				if (sub) {
					...
					sub.stoppedAt = Date.now();
					sub.lastActivityAt = sub.stoppedAt;
					sub._transitionedThisWindow = "stopped";
					sub._wasStoppedThisWindow = true;
	...
	/**
	 * Compute a lightweight hash of the mutable fields that matter for
	 * change detection.  Immutable identity fields (publicationName,
	 * connectionId, params, subscribedAt) are only sent on insert via
	 * $setOnInsert, so they don't need to trigger an update.
	 * @private
	 */
	_subscriptionHash(sub) {
		return `${sub.status}|${sub.documentsAdded}|${sub.documentsChanged}|${sub.documentsRemoved}|${sub.dataTransferred}|${sub.readyAt}|${sub.stoppedAt}|${sub.errorMessage || ""}`;
	}

	/**
	 * Send subscription updates to platform.
	 *
	 * Delta mode: only subscriptions whose mutable state changed since the
	 * last report cycle are included.  This eliminates 80-90% of writes on
	 * the server for apps with many stable "ready" subscriptions.
	 *
	 * A full snapshot is forced every `FULL_SNAPSHOT_CYCLES` cycles (default
	 * 10 ≈ 5 minutes at the default 30 s interval) so the server eventually
	 * catches up if a delta was lost.
	 * @private
	 */
	_sendSubscriptionUpdates() {
		...
		const windowStart = this.subscriptionWindowStart;

		// Apply server-hinted flush interval for NEXT window
		const hinted = this.client && this.client.serverConfig && this.client.serverConfig.subscriptions;
		if (hinted && hinted.flushIntervalMs && hinted.flushIntervalMs !== this.interval) {
			this._rescheduleInterval(hinted.flushIntervalMs);
		}

		if (!this.ingestAggregation) {
			this._sendLegacySubscriptionBatch();
			this._cleanupStoppedSubscriptions();
			this.subscriptionWindowStart = windowEnd;
			return;
		}
		...
		for (const sub of this.subscriptions.values()) {
			const paramsSignature = sub._cachedParamsSignature
				|| (sub._cachedParamsSignature = computeParamsSignature(sub.params || []));

			const key = `${sub.publicationName}|${paramsSignature}|${sub.status}|${sub._host || "unknown"}`;
			...
			if (!bucket) {
				bucket = {
					...
					paramsSignature,
					status: sub.status,
					host: sub._host || "unknown",
					paramsSample: buildSample(sub.params || []),
					subscriptionCount: 0,
					newSubscriptions: 0,
					stoppedSubscriptions: 0,
					_responseTimeSamples: [],
					documentsAddedSum: 0,
					documentsChangedSum: 0,
					documentsRemovedSum: 0,
					dataTransferredSum: 0,
					errorCount: 0
				};
				buckets.set(key, bucket);
			}
			bucket.subscriptionCount++;
			bucket.documentsAddedSum += sub.documentsAdded || 0;
			bucket.documentsChangedSum += sub.documentsChanged || 0;
			bucket.documentsRemovedSum += sub.documentsRemoved || 0;
			bucket.dataTransferredSum += sub.dataTransferred || 0;

			const transitionedIntoStatus = sub._transitionedThisWindow === sub.status;
			if (transitionedIntoStatus) bucket.newSubscriptions++;
			if (sub._wasStoppedThisWindow) bucket.stoppedSubscriptions++;

			if (sub.status === "ready" && sub._readyThisWindow && sub.responseTime != null) {
				bucket._responseTimeSamples.push(sub.responseTime);
			}

			if (sub.status === "error") bucket.errorCount++;

			const reasons = evaluateSubscription(sub);
			if (reasons.length > 0) {
				interestingEntities.push({
					...sub,
					_interestingReasons: reasons,
					_aggregateGroupKey: key
				});
				if (reasons.includes("long_lived_unreported")) {
					sub.fullReportedForLifecycle = true;
				}
			}

			sub._transitionedThisWindow = null;
			sub._wasStoppedThisWindow = false;
			sub._readyThisWindow = false;
			sub.previousStatus = sub.status;
		}

		const aggregates = Array.from(buckets.values()).map(b => ({
			publicationName: b.publicationName,
			paramsSignature: b.paramsSignature,
			status: b.status,
			host: b.host,
			paramsSample: b.paramsSample,
			subscriptionCount: b.subscriptionCount,
			newSubscriptions: b.newSubscriptions,
			stoppedSubscriptions: b.stoppedSubscriptions,
			responseTimeP50: percentile(b._responseTimeSamples, 0.5),
			responseTimeP95: percentile(b._responseTimeSamples, 0.95),
			responseTimeMax: b._responseTimeSamples.length ? Math.max(...b._responseTimeSamples) : null,
			documentsAddedSum: b.documentsAddedSum,
			documentsChangedSum: b.documentsChangedSum,
			documentsRemovedSum: b.documentsRemovedSum,
			dataTransferredSum: b.dataTransferredSum,
			errorCount: b.errorCount
		}));

		if (aggregates.length > 0 && this.client) {
			this.client.sendSubscriptionAggregates({
				windowStart: new Date(windowStart).toISOString(),
				windowEnd: new Date(windowEnd).toISOString(),
				flushIntervalMs: this.interval,
				sampleRate: 1.0,
				aggregates
			});
		}

		if (interestingEntities.length > 0 && this.client) {
			this.client.sendSubscriptions(interestingEntities);
		}

		this._log(`Subscription window flushed: ${aggregates.length} aggregates, ${interestingEntities.length} interesting`);
	...
	/**
	 * Legacy delta-based subscription emission path.
	 * @private
	 */
	_sendLegacySubscriptionBatch() {
		const FULL_SNAPSHOT_CYCLES = 10;
		this._snapshotCycleCounter = (this._snapshotCycleCounter || 0) + 1;
		const forceFullSnapshot = this._snapshotCycleCounter >= FULL_SNAPSHOT_CYCLES;
		if (forceFullSnapshot) {
			this._snapshotCycleCounter = 0;
		}

		const changed = [];
		this.subscriptions.forEach((sub) => {
			const hash = this._subscriptionHash(sub);
			if (forceFullSnapshot || hash !== sub._lastReportedHash) {
				sub._lastReportedHash = hash;
				changed.push(sub);
			}
		});

		if (this.client && changed.length > 0) {
			this.client.sendSubscriptions(changed);
			this._log(
				`Sent ${changed.length}/${this.subscriptions.size} subscription records` +
				(forceFullSnapshot ? " (full snapshot)" : " (delta)")
			);
		}
	}

	/** @private */
	_rescheduleInterval(newMs) {
		if (this.interval === newMs) return;
		this.interval = newMs;
		if (this.intervalId) clearInterval(this.intervalId);
		this.intervalId = setInterval(() => this._sendUpdates(), this.interval);
	}

	/** @private */
	_cleanupStoppedSubscriptions() {
		const now = Date.now();
