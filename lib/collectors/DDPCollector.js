...
export default class DDPCollector {
	...
	start() {
		...
		console.log(`✅ DDPCollector started (interval: ${this.interval}ms)`);
	}
	...
	stop() {
		...
		console.log("⏹️ DDPCollector stopped");
	}
	...
	_setupConnectionTracking() {
		...
		console.log('✅ DDPCollector: Polling for new sessions');
	...
	_wrapSession(session) {
		...
		try {
			...
			if (session.send) {
				...
				session.send = (message) => {
					try {
						...
						// Track subscription-related messages (like "ready")
			...
			console.log(`🔌 DDP Session wrapped: ${sessionId}`);
		} catch (error) {
	...
	_handleConnectionClose(connectionId) {
		...
		if (connData) {
			...
			connData.disconnectedAt = new Date();
		...
		try {
			if (msgType === "sub") {
				...
				if (subId && publicationName) {
					...
					console.log(`📡 DDPCollector: New subscription ${subId} to "${publicationName}"`);
				}
				...
				subs.forEach(subId => {
					...
					if (sub && sub.status === "pending") {
						sub.status = "ready";
						sub.readyAt = new Date();
						sub.responseTime = sub.readyAt - sub.subscribedAt;
						sub.lastActivityAt = new Date();

						console.log(`✅ DDPCollector: Subscription ${subId} ready (${sub.responseTime}ms)`);
					}
				});
				...
				if (sub) {
					...
					console.log(`❌ DDPCollector: Subscription ${subId} failed: ${sub.errorMessage}`);
				}
				...
				if (sub) {
					...
					console.log(`🛑 DDPCollector: Subscription ${subId} stopped`);
				}
	...
	_sendSubscriptionUpdates() {
		...
		if (this.client && subscriptions.length > 0) {
			...
			console.log(`📊 DDPCollector: Sent ${subscriptions.length} subscription records`);
		}
		...
		this.subscriptions.forEach((sub, subId) => {
			if ((sub.status === "stopped" || sub.status === "error") && sub.stoppedAt) {
				const age = now - sub.stoppedAt.getTime();
				if (age > 60000) {
