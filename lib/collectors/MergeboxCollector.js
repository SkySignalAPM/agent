 ...
 *   For a doc referenced by n handles, each referencing handle's bucket receives
 *   docBytes/n and 1/n docCount. Because the split divides by existsIn.size, the
 *   even-split rows for a collection SUM BACK to that collection's true residency
 *   (sum-preserving). That property is the load-bearing correctness invariant.
 *
 * STRATEGY:
 ...
 *   string. We reverse-map by identity against DDPServer.publicationStrategies
 *   (SERVER_MERGE / NO_MERGE / NO_MERGE_NO_HISTORY). NO_MERGE / NO_MERGE_NO_HISTORY
 *   keep NO collectionView entry at all (residency ~0); that absence is the whole
 *   signal and is never synthesized. NO_MERGE_MULTI uses DummyDocumentView (empty
 *   dataByKey) and maps to "unknown". Anything unrecognized / any throw -> "unknown".
 *
...
export default class MergeboxCollector {
	...
	_attributeSession(session, server, buckets) {
		...
		collectionViews.forEach((cview, collectionName) => {
			const documents = cview && cview.documents;
			if (!documents || typeof documents.forEach !== "function") {
				return; // shape mismatch — skip this collection view
			}
			...
			documents.forEach((docView) => {
				...
				// subs; even-split docBytes/docCount across them.
	...
	/**
	 * Materialize bucket map -> emit rows. Rounds fractional even-split values at
	 * emit time, computes connectionCount + avgBytesPerConnection, and caps to the
	 ...
	 * identity. Returns one of SERVER_MERGE | NO_MERGE | NO_MERGE_NO_HISTORY |
	...
	_resolveStrategyName(server, collectionName) {
		try {
			...
			// Identity reverse-map (only the three the schema accepts).
			...
			// NO_MERGE_MULTI and anything else are intentionally "unknown".
			return "unknown";
		} catch (_err) {
			return "unknown";
		}
	 ...
	 * shape to the schema enum. Only the three the schema accepts are returned.
	 * @private
	 */
	_structuralStrategyName(s) {
		...
		// NO_MERGE_MULTI (dummy view) and anything else -> unknown
		return "unknown";
	}
