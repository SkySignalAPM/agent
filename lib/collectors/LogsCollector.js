...
export default class LogsCollector {
	...
	_argToString(arg) {
		...
		// Objects and arrays — use JSON.stringify with a replacer that limits depth/size
		// to avoid expensive serialization of huge objects in hot console.log paths
		try {
			const result = JSON.stringify(arg);
			// Truncate immediately if very large — avoids holding a huge string in memory
			if (result.length > this.maxMessageLength) {
				return result.substring(0, this.maxMessageLength) + "...[truncated]";
			}
			return result;
		} catch (e) {
