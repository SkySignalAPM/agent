...
export default class SystemMetricsCollector {
  ...
  start() {
    ...
    try {
      this._collect(true); // skipSend = true
    } catch (error) {
    ...
    this.intervalId = setInterval(() => {
      this._collect(false); // skipSend = false
    }, this.interval);
  ...
  _collect(skipSend = false) {
    try {
      ...
      // Process count
      const processCount = this._getProcessCount();
   ...
   * Uses caching to reduce blocking execSync calls (disk stats change slowly)
   * @private
   */
  _getDiskStats() {
    const now = Date.now();

    // Return cached stats if still valid (avoid blocking execSync)
    ...
    try {
      ...
      if (this.platform === "linux") {
        // Linux: Read from df command
        const output = execSync("df -k /", { encoding: "utf8", timeout: 1000 });
  ...
  _getNetworkStats() {
    try {
      ...
      if (this.platform === "linux") {
        ...
        const output = execSync("netstat -ib -I en0", {
          encoding: "utf8",
          timeout: 1000,
        });
        const lines = output.trim().split("\n");
        ...
        // Windows: Use netsh to get per-interface statistics and sum all interfaces
        try {
          const output = execSync("netsh interface ip show subinterfaces", {
            encoding: "utf8",
            timeout: 1000,
          });
          const lines = output.trim().split("\n");
  ...
  _getProcessCount() {
    try {
      if (this.platform === "linux") {
        ...
        const output = execSync("ps ax | wc -l", {
          encoding: "utf8",
          timeout: 1000,
        });
        ...
        return parseInt(output.trim()) - 3; // Subtract header lines
      }
