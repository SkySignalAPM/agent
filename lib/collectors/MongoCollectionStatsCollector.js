...
export default class MongoCollectionStatsCollector {
	...
	_log(...args) {
		if (this.debug) {
			console.log('[SkySignal:CollectionStats]', ...args);
		}
	}
	...
	async _collectStats() {
		...
		try {
			...
			this._log(`Found ${collections.length} collections`);
			...
			this._log(`Collected stats for ${allStats.length} collections`);
			...
			if (allStats.length > 0) {
				...
				this._log('Stats sent to SkySignal');
			} else {
