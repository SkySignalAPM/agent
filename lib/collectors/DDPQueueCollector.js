...
export default class DDPQueueCollector {
  constructor(options = {}) {
    ...
    this.messageCache = {};
    this.messageCacheSize = 0; // Counter avoids Object.keys().length on every insert
  ...
  _log(...args) {
    if (this.debug) {
      console.log('[SkySignal:DDPQueue]', ...args);
    }
  }
  ...
  _wrapSession(session) {
    try {
      ...
      // Guard against double-wrapping the same session object (e.g. after
      // agent stop/restart where wrappedSessions Set is cleared but existing
      // sessions still carry previously wrapped handlers). See #5.
      if (session._skySignalDDPQueueWrapped) {
        return;
      }
      session._skySignalDDPQueueWrapped = true;
  ...
  _hijackMethodHandler(session) {
    ...
    session.protocol_handlers.method = function (msg, unblock) {
      ...
      // Call the version we captured (chains to other wrappers if present)
      // Note: Do NOT call unblock() in a finally block here. Meteor handles
      // method completion and queue advancement internally. Calling unblock()
      // unconditionally causes infinite recursion if this session was wrapped
  ...
  registerMessage(session, msg) {
    try {
      ...
      console.error('⚠️ DDPQueueCollector: Error registering message:', String(error));
    }
  }
  ...
  wrapUnblock(session, msg, originalUnblock) {
    ...
    return function () {
      ...
      // Collect metrics first, before calling originalUnblock.
      // Metrics collection is best-effort — failures must not prevent unblock.
      try {
        ...
        // Metrics failure is non-fatal; log and continue to unblock.
        // Use String(error) rather than the raw Error object to prevent
        // source-map-support's prepareStackTrace from re-overflowing when the
        // caught error already has a very deep stack trace (e.g. a RangeError
        // from mutual-recursion across wrapper layers). See #7.
        console.error('⚠️ DDPQueueCollector: Error recording unblock metrics:', String(error));
      } finally {
        // Always clear processing state, even if the catch block above
        // throws (e.g. console.error itself failing). This prevents the DDP
        // queue from getting permanently stuck. See #7.
        delete self.currentProcessing[session.id];

        // Call original unblock asynchronously via queueMicrotask. This breaks
        // any potential synchronous recursion chain when another APM agent
        // (e.g. montiapm:agent) also wraps the unblock function and proxies
        // it synchronously. Meteor's own runHandlers() independently calls its
        // native unblock after the protocol handler returns, so deferring our
        // call here does not delay queue advancement. See #7.
        if (typeof originalUnblock === 'function') {
          queueMicrotask(() => {
            try {
              originalUnblock();
            } catch (_e) {
              // originalUnblock may be another agent's wrapper that throws.
              // Swallow to prevent unhandled microtask errors — the DDP queue
              // is advanced by Meteor's own unblock call, not ours.
            }
          });
        }
      }
  ...
  calculateWaitedOn(session, startTime) {
    ...
    try {
      ...
      console.error('⚠️ DDPQueueCollector: Error calculating waitedOn:', String(error));
    }
  ...
  _cacheMessage(session, msg) {
    ...
    if (!this.messageCache[key]) {
      // Cleanup old entries if cache is too large (uses counter, avoids Object.keys().length)
      if (this.messageCacheSize >= this.messageCacheMaxSize) {
  ...
  _cleanupMessageCache(now = Date.now()) {
    ...
    if (remaining >= this.messageCacheMaxSize) {
    ...
    // Sync the counter after cleanup
    this.messageCacheSize -= removed;
  }
