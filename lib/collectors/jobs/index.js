...
import BullMQMonitor from "./BullMQMonitor.js";
...
export class JobCollector {
	...
	static create(options = {}) {
		...
		const _log = (...args) => {
			if (options.debug) console.log('[SkySignal:Jobs]', ...args);
		};
		...
		if (options.preferredPackage && this.ADAPTERS[options.preferredPackage]) {
			...
			if (adapter.isPackageAvailable()) {
				_log(`Using job monitor for: ${options.preferredPackage}`);
				return adapter;
			} else {
		...
		for (const [packageName, AdapterClass] of Object.entries(this.ADAPTERS)) {
			try {
				...
				if (adapter.isPackageAvailable()) {
					_log(`Auto-detected job package: ${packageName}`);
					return adapter;
				}
			} catch (e) {
		...
		_log("No supported job queue package detected - job monitoring disabled");
		return null;
	}
	...
	static registerAdapter(packageName, adapterClass) {
		...
		// Registration is a rare event, keep this as a debug-guarded log would require
		// passing debug state to a static method; adapters log on start() instead.
	}
}

// Export individual classes for advanced usage
export { BaseJobMonitor, SteveJobsMonitor, BullMQMonitor };
