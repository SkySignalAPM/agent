...
export default class BaseJobMonitor {
	...
	start() {
		if (!this.enabled) {
			console.log(`⏸️  ${this.constructor.name} disabled`);
			return;
		}
		...
		this.started = true;
		console.log(`✅ ${this.constructor.name} started (interval: ${this.interval}ms)`);
	}
	...
	_sendJobEvent(job) {
		...
		console.log(`📤 Sending job event: ${job.jobName} (${job.jobId}) - status: ${job.status}`);
