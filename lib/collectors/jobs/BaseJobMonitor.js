...
export default class BaseJobMonitor {
	...
	trackJobStart(jobInfo) {
		...
		const job = {
			...
			queueName: jobInfo.queueName || null,
			status: "running",
			...
			progress: 0,
			originatingTraceId: jobInfo.originatingTraceId || null
		};
	...
	_sendJobEvent(job) {
		...
		const payload = {
			...job,
			jobsPackage: this.getPackageName()
		};

		// Add the job to the jobs batch
		this.client._addToBatch("jobs", payload, "/api/v1/jobs");
	}
