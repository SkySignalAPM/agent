...
export default class LiveQueriesCollector {
	...
	_setupObserverTracking() {
		...
		try {
			...
			this._log("Observer tracking enabled (driver detection: per-observer)");
		} catch (error) {
	...
	_trackObserver(collectionName, selector, options, type, handle, driverType, callbacks) {
		...
		if (existingObserver) {
			...
			this._log(`Reusing observer: ${collectionName} (${existingObserver.handlersSharing} handlers) - ${existingObserver.observerId}`);
		...
		this._log(`Tracking new observer: ${collectionName} (${driverLabel[observerData.observerType] || observerData.observerType}) - ${observerId}`);
	...
	_wrapObserveCallbacks(observerData, callbacks) {
		...
		if (callbacks.added) {
			...
			wrapped.added = function(document) {
				...
				observerData.lastActivityAt = Date.now();
