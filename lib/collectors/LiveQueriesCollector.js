...
import { AggregateBucket } from "./aggregation/AggregateBucket.js";
import { computeQuerySignature, buildSample } from "./aggregation/QuerySignature.js";
import { evaluateObserver } from "./aggregation/ObserverPromotion.js";

/**
 * AsyncLocalStorage for propagating publication context to _observeChanges.
 * PublicationTracer wraps Meteor.publish handlers in publicationContextStore.run()
 * so that when _observeChanges fires inside a publication, we can read the
 * publication name, connectionId, and isAutoPublish flag.
 * Exported for use by PublicationTracer.
 */
export const publicationContextStore = new AsyncLocalStorage();
...
export default class LiveQueriesCollector {
	constructor(options = {}) {
		...
		this._observedNew = new Set();
		this._observedStopped = new Set();
	}
			...
			if (hasObserveChanges) {
				...
				MongoInternals.Connection.prototype._observeChanges = async function(
					...
					if (cursorDescription?.collectionName) {
						...
						// Read publication context from AsyncLocalStorage
						// (set by PublicationTracer._wrapPublish when inside a publish handler)
						const pubContext = publicationContextStore.getStore();
						if (pubContext) {
							observerRef.data.publicationName = pubContext.publicationName || null;
							observerRef.data.connectionId = pubContext.connectionId || null;
							observerRef.data.isAutoPublish = pubContext.isAutoPublish || false;
						}
					}
	...
	_finalizeObserver(observerData, handle, driverType) {
		...
		// Track as new-this-window for aggregation mode
		this._observedNew.add(observerData.observerId);
	}
	...
	_extractCursorMetadata(observerData, cursorDescription) {
		try {
			...
			// Sort-shape (used for query signature, not values)
			if (opts.sort && typeof opts.sort === "object") {
				observerData._sortKeys = opts.sort;
			}
		...
		handle.stop = function() {
			...
			if (observerId) {
				...
				if (observer) {
					...
					self._observedStopped.add(observerId);
	...
	_sendUpdates() {
		...
		const hinted = this.client && this.client.serverConfig && this.client.serverConfig.liveQueries;
		if (hinted && hinted.flushIntervalMs && hinted.flushIntervalMs !== this.interval) {
			this._log(`Server-hinted flushIntervalMs: ${hinted.flushIntervalMs}`);
			this._rescheduleInterval(hinted.flushIntervalMs);
		}

		if (!this.ingestAggregation) {
			this._sendLegacyBatch();
			this._cleanupStoppedObservers();
			this._resetWindowState(windowEnd);
			return;
		}
		...
		for (const obs of this.observers.values()) {
			...
			if (obs.status === "active") this._snapshotDriverMetrics(obs);
			obs.performance = this._calculatePerformance(obs);

			const querySignature = obs._cachedQuerySignature
				|| (obs._cachedQuerySignature = computeQuerySignature(obs.query || {}, {
					limit: obs.queryLimit,
					sort: obs._sortKeys,
					fields: obs.hasProjection ? {} : undefined
				}));

			const identity = {
				collectionName: obs.collectionName,
				publicationName: obs.publicationName || null,
				observerType: obs.observerType,
				querySignature,
				host: obs.host,
				isAutoPublish: !!obs.isAutoPublish,
				querySample: buildSample(obs.query || {}),
				hasLimit: !!obs.hasLimit,
				queryLimit: obs.queryLimit || 0,
				hasProjection: !!obs.hasProjection
			};
			const groupKey = [
				identity.collectionName,
				identity.publicationName,
				identity.observerType,
				identity.querySignature,
				identity.host,
				identity.isAutoPublish
			].join("|");

			let bucket = buckets.get(groupKey);
			if (!bucket) {
				bucket = new AggregateBucket({ groupKey, identity });
				buckets.set(groupKey, bucket);
			}
			bucket.addObserver(obs, {
				isNew: this._observedNew.has(obs.observerId),
				isStopped: this._observedStopped.has(obs.observerId)
			});

			const reasons = evaluateObserver(obs, { windowDurationMs });
			if (reasons.length > 0) {
				interestingEntities.push(this._buildEntityPayload(obs, reasons, groupKey));
			}
		}

		if (buckets.size > 0 && this.client) {
			const aggregatePayload = {
				windowStart: new Date(windowStart).toISOString(),
				windowEnd: new Date(windowEnd).toISOString(),
				flushIntervalMs: this.interval,
				sampleRate: 1.0,
				aggregates: Array.from(buckets.values()).map(b => b.toAggregateRow())
			};
			this.client.sendLiveQueryAggregates(aggregatePayload);
		}

		if (interestingEntities.length > 0 && this.client) {
			this.client.sendLiveQueries(interestingEntities);
		}

		this._log(`Window flushed: ${buckets.size} aggregate rows, ${interestingEntities.length} interesting entities`);
	...
	/**
	 * Legacy per-observer emission path. Pre-aggregation behavior.
	 * @private
	 */
	_sendLegacyBatch() {
		const observers = Array.from(this.observers.values()).map(obs => {
			this._updateActivityRate(obs);
			if (obs.status === "active") this._snapshotDriverMetrics(obs);
	...
	/**
	 * Build an interesting-entity payload used in aggregation mode when a
	 * per-observer record needs to be shipped alongside the rollup.
	 * @private
	 */
	_buildEntityPayload(obs, reasons, aggregateGroupKey) {
		return {
			observerId: obs.observerId,
			collectionName: obs.collectionName,
			query: obs.query,
			...
			avgProcessingTime: obs.avgProcessingTime,
			backlogSize: obs.backlogSize,
			observerLifespan: Math.round((Date.now() - obs.createdAt) / 1000),
			createdAt: obs.createdAt,
			stoppedAt: obs.stoppedAt,
			lastActivityAt: obs.lastActivityAt,
			timestamp: Date.now(),
			...
			_aggregateGroupKey: aggregateGroupKey
		};
	}

	/** @private */
	_resetWindowState(windowEnd) {
		this.windowStartTime = windowEnd;
		this._observedNew.clear();
		this._observedStopped.clear();
	}
