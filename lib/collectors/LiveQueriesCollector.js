 ...
 *
 * Monitors:
 * - Observer type (oplog tailing vs polling)
...
export default class LiveQueriesCollector {
	...
	_setupObserverTracking() {
		...
		try {
			// Determine if oplog is available
			const isOplogAvailable = !!process.env.MONGO_OPLOG_URL;
			...
			Mongo.Collection.prototype.find = function(...args) {
				...
				cursor.observe = function(callbacks) {
					...
					self._trackObserver(collectionName, args[0], args[1], "observe", handle, isOplogAvailable, callbacks);
				...
				cursor.observeChanges = function(callbacks) {
					...
					self._trackObserver(collectionName, args[0], args[1], "observeChanges", handle, isOplogAvailable, callbacks);
					return self._wrapHandle(handle);
				};
			...
			console.log(`✅ Observer tracking enabled (oplog: ${isOplogAvailable ? 'yes' : 'no'})`);
		} catch (error) {
	 ...
	 * @private
	 */
	_trackObserver(collectionName, selector, options, type, handle, isOplogAvailable, callbacks) {
		...
		const observerData = {
			...
			observerType: isOplogAvailable ? "oplog" : "polling",
			isOplogEfficient: isOplogAvailable,
			observerCount: 1,
			handlersSharing: 1,
			documentCount: 0,
			fetchedDocuments: 0,
		...
		console.log(`📡 Tracking new observer: ${collectionName} (${observerData.observerType}) - ${observerId}`);
	...
	_calculatePerformance(observer) {
		if (observer.observerType === "oplog") {
			...
			const { optimal, good, inefficient } = this.thresholds.polling;
	...
	getStats() {
		...
		return {
			...
			oplogEfficiency:
				activeObservers.length > 0 ? Math.round((oplogObservers.length / activeObservers.length) * 100) : 100,
