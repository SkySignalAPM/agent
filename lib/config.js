 ...
 *
 * @property {number} batchSize - Maximum items per batch before auto-flush (default: 100)
 * @property {number} batchSizeBytes - Maximum batch size in bytes (default: 1MB)
 * @property {number} flushInterval - Auto-flush interval in milliseconds (default: 60000)
 *
 ...
 * @property {number} indexUsageSampleRate - Sampling rate for explain(), 0.0-1.0 (default: 0.01 = 1%)
 ...
 * @property {number} requestTimeout - HTTP request timeout in milliseconds (default: 30000)
...
export const DEFAULT_CONFIG = {
	// API Configuration
	apiKey: null,
	endpoint: "https://dash.skysignal.app",
	enabled: true,

	// Debug Mode (logs verbose output to console, opt-in via settings)
	debug: false,
	...
	batchSizeBytes: 1024 * 1024,      // Max 1MB per batch
	...
	ddpConnectionsInterval: 60000,    // 60 seconds (DDP connection updates)
	...
	indexUsageSampleRate: 0.01,       // Sample 1% of queries for explain()
	explainVerbosity: 'queryPlanner',   // queryPlanner (plan only) | executionStats (re-executes) | allPlansExecution
	explainSlowQueriesOnly: true,     // Only explain queries exceeding slowQueryThreshold
	...
	jobsPackage: null,                // Auto-detect, or specify: "msavin:sjobs", "bullmq"

	// BullMQ-specific configuration
	bullmqQueues: [],                 // Manual queue configs: [{name: 'myQueue', connection: {host, port}}]
	bullmqRedis: null,                // Default Redis connection: {host: 'localhost', port: 6379}

	// Log Collection
	collectLogs: true,                // Enable log capturing
	logLevels: ["info", "warn", "error", "fatal"],  // Levels to capture (excludes debug by default)
	logSampleRate: 1.0,               // 100% capture rate (reduce for high-volume apps)
	logMaxMessageLength: 5000,        // Max chars per log message
	logCaptureConsole: true,          // Wrap console.log/info/warn/error/debug
	logCaptureMeteorLog: true,        // Wrap Meteor Log.info/warn/error/debug

	// DNS Timing
	collectDnsTimings: true,          // Enable DNS resolution latency tracking
	dnsTimingsInterval: 60000,        // 60 seconds

	// Outbound HTTP (diagnostics_channel)
	collectOutboundHttp: true,        // Enable outbound HTTP client metrics
	outboundHttpInterval: 60000,      // 60 seconds

	// CPU Profiling (on-demand)
	collectCpuProfiles: true,         // Enable on-demand CPU profiling
	cpuProfileThreshold: 80,          // CPU % threshold to trigger profiling
	cpuProfileDuration: 10000,        // 10 second profile duration
	cpuProfileCooldown: 300000,       // 5 minutes between profiles
	cpuProfileCheckInterval: 60000,   // Check CPU every 60 seconds

	// Deprecated API Usage Detection
	collectDeprecatedApis: true,      // Enable sync vs async API tracking
	deprecatedApisInterval: 300000,   // 5 minutes

	// Publication Over-Fetching Detection
	collectPublications: true,        // Enable publication efficiency tracking
	publicationsInterval: 300000,     // 5 minutes

	// Environment Metadata
	collectEnvironment: true,         // Enable environment snapshot collection
	environmentInterval: 1800000,     // 30 minutes

	// Live Queries (Observer Tracking)
	collectLiveQueries: true,         // Enable reactive query observer monitoring
	liveQueriesInterval: 60000,       // 60 seconds
	liveQueriesMaxObservers: 5000,    // Max tracked observers before eviction (min: 500)
	liveQueriesPerformanceThresholds: null, // Optional: custom performance thresholds

	// Vulnerability Scanning
	collectVulnerabilities: true,     // Enable npm audit vulnerability scanning
	vulnerabilitiesInterval: 3600000, // 1 hour

	// Mergebox RAM Residency (ships dark — opt-in)
	collectMergebox: false,           // Enable mergebox residency attribution (default OFF)
	mergeboxInterval: 60000,          // 60 seconds (low cadence — a full session walk)
	mergeboxSampleRate: 1.0,          // Per-session sample rate, 0 < rate <= 1
	mergeboxMaxSessions: 2000,        // Cap sessions walked per tick (bounds the sync tick)
	mergeboxMaxDocsPerSession: 50000, // Cap docs walked per session per tick

	// Aggregation Ingest (requires platform v1.0.30+)
	ingestAggregation: true           // Roll up live query / subscription telemetry into fixed-shape aggregates before shipping
};
...
export function validateConfig(config) {
	check(config, {
		apiKey: Match.OneOf(String, null),
		endpoint: Match.Optional(String),
		enabled: Match.Optional(Boolean),
		debug: Match.Optional(Boolean),
		host: Match.Optional(Match.OneOf(String, null)),
		...
		// BullMQ
		bullmqQueues: Match.Optional([Match.ObjectIncluding({
			name: String,
			connection: Match.Optional(Object)
		})]),
		bullmqRedis: Match.Optional(Match.OneOf(Object, null)),

		collectLogs: Match.Optional(Boolean),
		logLevels: Match.Optional([String]),
		logSampleRate: Match.Optional(Number),
		logMaxMessageLength: Match.Optional(Match.Integer),
		logCaptureConsole: Match.Optional(Boolean),
		logCaptureMeteorLog: Match.Optional(Boolean),

		// DNS Timing
		collectDnsTimings: Match.Optional(Boolean),
		dnsTimingsInterval: Match.Optional(Match.Integer),

		// Outbound HTTP
		collectOutboundHttp: Match.Optional(Boolean),
		outboundHttpInterval: Match.Optional(Match.Integer),

		// CPU Profiling
		collectCpuProfiles: Match.Optional(Boolean),
		cpuProfileThreshold: Match.Optional(Number),
		cpuProfileDuration: Match.Optional(Match.Integer),
		cpuProfileCooldown: Match.Optional(Match.Integer),
		cpuProfileCheckInterval: Match.Optional(Match.Integer),

		// Deprecated API Detection
		collectDeprecatedApis: Match.Optional(Boolean),
		deprecatedApisInterval: Match.Optional(Match.Integer),

		// Publication Efficiency
		collectPublications: Match.Optional(Boolean),
		publicationsInterval: Match.Optional(Match.Integer),

		// Environment Metadata
		collectEnvironment: Match.Optional(Boolean),
		environmentInterval: Match.Optional(Match.Integer),

		// Live Queries
		collectLiveQueries: Match.Optional(Boolean),
		liveQueriesInterval: Match.Optional(Match.Integer),
		liveQueriesMaxObservers: Match.Optional(Match.Integer),
		liveQueriesPerformanceThresholds: Match.Optional(Match.OneOf(Object, null)),

		// Vulnerability Scanning
		collectVulnerabilities: Match.Optional(Boolean),
		vulnerabilitiesInterval: Match.Optional(Match.Integer),

		// Mergebox RAM Residency
		collectMergebox: Match.Optional(Boolean),
		mergeboxInterval: Match.Optional(Match.Integer),
		mergeboxSampleRate: Match.Optional(Number),
		mergeboxMaxSessions: Match.Optional(Match.Integer),
		mergeboxMaxDocsPerSession: Match.Optional(Match.Integer),

		// Aggregation Ingest
		ingestAggregation: Match.Optional(Boolean)
	});
	...
	if (config.liveQueriesMaxObservers !== undefined && config.liveQueriesMaxObservers < 500) {
		throw new Error("liveQueriesMaxObservers must be at least 500");
	}

	// Mergebox: sampleRate in (0, 1]; interval and caps must be positive ints.
	if (config.mergeboxSampleRate !== undefined && (config.mergeboxSampleRate <= 0 || config.mergeboxSampleRate > 1)) {
		throw new Error("mergeboxSampleRate must be greater than 0 and at most 1");
	}

	if (config.mergeboxInterval !== undefined && config.mergeboxInterval < 1000) {
		throw new Error("mergeboxInterval must be at least 1000ms");
	}

	if (config.mergeboxMaxSessions !== undefined && config.mergeboxMaxSessions < 1) {
		throw new Error("mergeboxMaxSessions must be at least 1");
	}

	if (config.mergeboxMaxDocsPerSession !== undefined && config.mergeboxMaxDocsPerSession < 1) {
		throw new Error("mergeboxMaxDocsPerSession must be at least 1");
	}

	if (config.logSampleRate !== undefined && (config.logSampleRate < 0 || config.logSampleRate > 1)) {
		throw new Error("logSampleRate must be between 0 and 1");
	}

	if (config.logLevels !== undefined) {
 ...
 * console.log(config.flushInterval); // 60000 (default)
